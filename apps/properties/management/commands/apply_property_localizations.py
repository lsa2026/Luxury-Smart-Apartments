        ...
        for listing_id, values in content.items():
            ...
            if not options["dry_run"]:
                for field_name in changed_fields:
                    value_key = (
                        PUBLIC_VISIBILITY_FIELD if field_name == "is_visible" else field_name
                    )
...
def _load_content() -> dict[str, dict[str, Any]]:
    ...
    for listing_id, values in payload.items():
        ...
        if PUBLIC_VISIBILITY_FIELD in values and not isinstance(
            values[PUBLIC_VISIBILITY_FIELD], bool
        ):
            raise CommandError(
