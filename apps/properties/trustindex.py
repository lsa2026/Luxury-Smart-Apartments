...
def widget_mapping() -> dict[str, dict[str, object]]:
    """Load the reviewed mapping once per application process."""
...
def full_review_widget_id(property_obj: object, language: str | None = None) -> str:
    """Return the property-specific full-list widget in the current language."""

    listing_id = str(getattr(property_obj, "hostaway_listing_id", ""))
    config = widget_mapping().get(listing_id, {})
...
def metrics_widget_id(property_obj: object) -> str:
    """Use the complete English list as the aggregate-rating source."""
