"""Read the public aggregate shown by an approved Trustindex widget.

Trustindex remains the source of review text and attribution in the browser.
The local database stores only its visible aggregate so cards, sorting and
structured data do not have to query a third party during a guest request.
"""
...
class TrustindexFetchError(RuntimeError):
    """Raised when the public widget cannot provide a safe aggregate."""
...
def _widget_url(widget_id: str) -> str:
    ...
    return f"https://cdn.trustindex.io/widgets/{widget_id[:2].lower()}/{widget_id}/content.html"


def fetch_widget_metrics(widget_id: str, *, timeout: float = 15.0) -> TrustindexMetrics:
    """Return the score and count rendered at the top of a Trustindex widget."""
    origin = settings.SITE_CANONICAL_URL.rstrip("/") + "/"
    request = Request(
        _widget_url(widget_id),
        headers={
    ...
    try:
        with urlopen(request, timeout=timeout) as response:  # noqa: S310 - fixed Trustindex origin
    ...
    if rating > Decimal("5") and max_rating_match:
        maximum = Decimal(max_rating_match.group(1))
        rating = (rating * Decimal("5") / maximum).quantize(
            Decimal("0.1"),
    ...
    return TrustindexMetrics(rating=rating, review_count=review_count)
