...
## اختبار SMTP محلي آمن

بيئة التطوير تعرض البريد في الطرفية افتراضيًا، حتى لو وُجدت إعدادات SMTP. لاختبار
حساب بريد الأعمال بصورة مقصودة، أضيفي هذا المتغير المحلي فقط ثم أرسلي رسالة اختبار
إلى صندوق داخلي تملكينه:

```env
EMAIL_LOCAL_SMTP_ENABLED=True
```

اتركي `EMAIL_DELIVERY_ENABLED=False` أثناء اختبار اتصال SMTP المباشر؛ بهذا لا يمكن
لأي تصفح محلي أو حدث حجز أن يرسل بريدًا تلقائيًا إلى ضيف.
