...
## اختبار مرحلة المعاينة بأمان

عند اختبار عامل البريد على Render، استخدم قائمة استقبال مؤقتة ومحددة:

```env
EMAIL_TEST_RECIPIENT_ALLOWLIST=saeed@luxurysmartapartments.com
```

بوجود هذه القيمة، يرفض النظام من طبقة الطابور ثم من مزوّد البريد نفسه أي عنوان آخر؛
لذلك لا يمكن أن تصل رسالة اختبار إلى ضيف. هذه ليست إعدادًا للإنتاج: بعد اعتماد رحلة
البريد الفعلية تُزال القائمة فقط ضمن خطة إطلاق منفصلة ومراجعة مكتملة.
